...
pcregrep -i -M "(?<!it)[\W]+(is|am)([\s,()]+not?)?([\s,()]+a)?[\s,()]+([a-z]+-)?(${titles})[s-]?\b" $1/*

# "are", optional "no" or "not", optional "a", optionally prefixed "(bachelor|gentleman)"
pcregrep -i -M "\b(ar[et])([\s,()]+not?)?([\s,()]+a)?[\s,()]+([a-z]+-)?(${titles})[s-]?\b" $1/*

# "be", optional "no" or "not", optional "a", "(bachelor|gentleman)" 
pcregrep -i -M "\b(be)([\s,()]+not?)?([\s,()]+a)?[\s,()]+([a-z]+-)?(${titles})[s-]?\b" $1/*
...
pcregrep -i -M "\b(I|you|s?he|thou|[w|y]e|they)([\s,()]+not?)?([\s,()]+a)?[\s,()]+([a-z]+-)?(${titles})[s-]?\b" $1/*

# seeming
pcregrep -i -M "\bseems?([\s,()]+not?)?([\s,()]+a)?[\s,()]+(${titles})s?\b" $1/*
